return {
	...
	},
	{
		...
		dependencies = {
			-- Automatically install LSPs and related tools to stdpath for
			-- Neovim Mason must be loaded before its dependents so we need to
			-- set it up here. NOTE: `opts = {}` is the same as calling
			-- `require('mason').setup({})`
		...
		config = function()
			...
			--
			-- LSP stands for Language Server Protocol. It's a protocol that
			-- helps editors and language tooling communicate in a standardized
			-- fashion.
			--
			...
			-- processes that communicate with some "client" - in this case,
			-- Neovim!
			--
			-- LSP provides Neovim with features like: - Go to definition -
			-- Find references - Autocompletion - Symbol Search - and more!
			--
			-- Thus, Language Servers are external tools that must be installed
			-- separately from Neovim. This is where `mason` and related
			-- plugins come into play.
			--
			-- If you're wondering about lsp vs treesitter, you can check out
			-- the wonderfully and elegantly composed help section, `:help
			-- lsp-vs-treesitter`

			--  This function gets run when an LSP attaches to a particular
			--  buffer. That is to say, every time a new file is opened that is
			--  associated with an lsp (for example, opening `main.rs` is
			--  associated with `rust_analyzer`) this function will be executed
			...
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(event)
					...
					-- It sets the mode, buffer and description for us each
					-- time.
